...
**Transport decision (matches the deployment):** the private instance bearer-gates `/api/` by an `Authorization: Bearer` **header** at the reverse proxy, and Task 5's `/live` handler deliberately has no `?token=` support (unauthenticated at the app layer, like its siblings). `EventSource` cannot set headers, so use a **`fetch`-based SSE reader with the `Authorization` header** — it passes the existing proxy gate, reuses the bearer/401-retry flow, and needs no backend or nginx change. Reconnect manually (a loop with backoff; `fetch` streaming has no native reconnect).

**Session-boundary decision:** Task 5 flagged that the `watch` watermark can coalesce and DROP the discrete `rollover` event under a fast double-tick. So the client must NOT rely on the `rollover` event for correctness — instead derive the boundary from the item stream itself: track `currentSession`; an incoming item whose `session_index` exceeds `currentSession` starts a fresh list (the previous session sealed). Treat any `rollover` event as a non-authoritative hint only (e.g. a brief "sealed" badge). This is robust to coalesced/dropped watermarks.

**Files:**
- Modify: `fmo_frontend_react/src/services/api.ts` (add `subscribeLive`, an injectable-for-tests fetch-stream subscriber) + reuse `src/types/api.ts`'s `SessionItem`
- Create: `src/components/explorer/LiveView.tsx`
- Modify: `src/pages/FederationDetail.tsx` (a "Live" tab)
- Test: `src/components/explorer/LiveView.test.tsx` + `src/services/api.test.ts` (extend, for `subscribeLive` frame parsing)

- [ ] **Step 1: `subscribeLive` in `api.ts`.** `export function subscribeLive(federationId: string, handlers: { onItem: (i: SessionItem) => void; onRollover?: (sessionIndex: number) => void; onError?: (e: unknown) => void }, signal: AbortSignal): void` — `fetch(`${BASE_URL}/federations/${federationId}/live`, { headers: { Authorization: `Bearer ${token}` }, signal })` using `auth.getToken()` (on a 401, `await auth.ensureToken()` and retry once, mirroring `authedFetch`). Read `res.body!.getReader()` + `TextDecoder`; buffer and split SSE frames on `\n\n`; for each frame parse the `event:` line (default `message`) and concatenated `data:` line(s): a `message` frame's data is JSON → `onItem(JSON.parse(data))`; an `event: rollover` frame → `onRollover?(Number(data))`. On stream end/error, if not aborted, reconnect after a short backoff (e.g. 1s). Abort (via the `signal`) stops cleanly. Keep the SSE-frame parsing in a small exported helper so the test can feed frames directly.
- [ ] **Step 2: `LiveView` component.** `LiveView({ federationId }: { federationId: string })`: on mount, create an `AbortController`, call `subscribeLive(federationId, handlers, controller.signal)`, abort on unmount. State: `items: SessionItem[]`, `currentSession: number | null`. `onItem(i)`: if `currentSession === null || i.session_index > currentSession` → set `currentSession = i.session_index` and `items = [i]` (new session sealed the old list); else if `i.session_index === currentSession` → append (dedupe by `${session_index}-${item_index}` to tolerate an overlapping reconnect read). Render SP‑1's `<ItemList items={items} scope="consensus" onLoadMore={() => {}} hasMore={false} />`, a "● LIVE" indicator (pulsing dot) + `Session {currentSession ?? '—'}`, and an idle/empty hint before the first item. `onError` sets a small non-fatal banner but the loop keeps retrying.
- [ ] **Step 3: Wire a "Live" tab** into `FederationDetail`: extend the `activeTab` union (`'activity' | 'sessions' | 'consensus' | 'utxos' | 'config'`) with `'live'`, add a tab button matching the existing button styling/pattern (see the `sessions`/`consensus` buttons ~lines 530-548), and render `{activeTab === 'live' && <LiveView federationId={federationId} />}` in the tab body. Import `LiveView` (lazy is fine but not required).
- [ ] **Step 4: Tests.** In `api.test.ts`, unit-test the exported SSE-frame parser: feed `data: {json}\n\n` → one `onItem`; `event: rollover\ndata: 5\n\n` → `onRollover(5)`; a split-across-chunks frame reassembles. In `LiveView.test.tsx`, inject a fake subscriber (mock `subscribeLive` via `vi.mock('../../services/api', …)`) and drive `onItem`: assert items render via `ItemList`; an item with a higher `session_index` resets the list to just that item (boundary derived from the stream, NOT a rollover event). `npm test` + `npm run build`.
- [ ] **Step 5: Commit.**
