...
/// Resolves `user_tx_key` + `user_tx_kind` + `direction` for a `transactions`
/// row aliased `t` in the enclosing query, via a single LATERAL join --
/// avoiding row multiplication if a txid ever maps to more than one
/// `user_tx_key` (`LIMIT 1`) and avoiding three separate correlated
