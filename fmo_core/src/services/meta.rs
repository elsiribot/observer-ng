...
/// Renders a client config as JSON, decoding module configs with the decoders
/// provided by the installed observer modules; unknown module kinds are
/// rendered as raw hex.
pub fn config_to_json(
    cfg: ClientConfig,
    registry: &ModuleRegistry,
) -> anyhow::Result<JsonClientConfig> {
    let decoders = registry.decoders(&cfg);
