//! Vendored consensus types for the fedi `multi_sig_stability_pool` module.
//!
//! Copied verbatim (minus the upstream proptest unit tests and the
//! `endpoint_constants` submodule) from fedi's `stability-pool-common` crate:
//! <https://github.com/fedixyz/fedi> `crates/modules/stability-pool/common`
//! (crate `stability-pool-common` v0.3.0, module consensus version 2.1).
//!
//! We vendor rather than depend on the crate directly because
//! `stability-pool-common` pins `fedimint-core` to the fedibtc fork
//! (`github.com/fedibtc/fedimint` tag `v0.11.0-fedi9`, version `0.11.0-rc.1`),
//! a different git source than this workspace's upstream `fedimint/fedimint`
//! v0.11.1. Depending on it would pull a second, incompatible `fedimint-core`,
//! so its `Decoder`/`DynInput`/`Decodable` types would not be assignable to the
//! observer's. The consensus encoding of these module types is defined purely
//! by fedimint-core's `Encodable`/`Decodable` derives plus primitive encodings
//! (`Amount`, `PublicKey`, `sha256::Hash`, `SystemTime`, ...), which are stable
//! across fedimint 0.11.x, so this copy decodes production bytes identically.
...
// Upstream proptest unit tests removed in the vendored copy; the observer
// exercises the decoder in fmo_module_stability_pool/tests/process.rs.
