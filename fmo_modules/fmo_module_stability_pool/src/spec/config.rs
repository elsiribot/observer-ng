//! Vendored config types for `multi_sig_stability_pool`. See the module doc in
//! `spec.rs` for why these are vendored rather than depended upon. Copied from
//! fedi's `stability-pool-common` crate, `src/config.rs`.
